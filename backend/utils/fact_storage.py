...
async def _process_isolated_ledger_write(session_id: str, epoch: int, event_type: str, rolling_summary: str):
    """
...
def trigger_on_demand_save(session_id: str, epoch: int, event_type: str, rolling_summary: str):
    """
    LAYER 1 ISOLATION: Event Loop Detachment
    Returns control to the UI stream instantly.
    """
    if not session_id:
        return

    asyncio.create_task(
        _process_isolated_ledger_write(session_id, epoch, event_type, rolling_summary)
    )