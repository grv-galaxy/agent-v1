...
from src.search_agent.planner import generate_sub_queries
...
async def execute_sub_pipeline(
    ...
    class WsWrapper:
        async def send_json(self, data):
            if not emit_ws_events and data.get("type") in ["stage_start", "stage_done", "source_start", "source_done", "page_visit_start", "page_visit_done"]:
                return
            await _original_ws.send_json(data)
    ...
    # --- Evaluate Deep Research Trigger (Phase 1) ---
    is_deep_research = deep_research_override or (not is_diagnostic_mode and classifier_out.requires_deep_research)
    if is_deep_research:
        trigger_type = "user_override" if deep_research_override else "planner"
        reason = "Manually requested by user" if deep_research_override else classifier_out.deep_research_reasoning
        await websocket.send_json({
            "type": "deep_research_activated",
            "trigger": trigger_type,
            "reason": reason
        })
        
        # Phase 2: Generate sub-queries (log only for now, no multi-hop fetching yet)
        print(f"[Deep Research] Generating sub-queries for: {query}")
        planner_out, plan_telemetry = await generate_sub_queries(query)
        plan_telemetry["query_id"] = query_id
        plan_telemetry["sub_query_id"] = sub_query_id
        llm_calls_to_log.append(plan_telemetry)
        print(f"[Deep Research] Generated sub-queries: {planner_out.sub_queries}")
    ...
    if not search_results:
        ...
        return {'top_3': [], 'metrics': metrics, 'tool_contributions': tool_contributions, 'llm_calls_to_log': llm_calls_to_log}
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            deep_research_override = payload.get("deep_research", False)
            if not query:
                continue
